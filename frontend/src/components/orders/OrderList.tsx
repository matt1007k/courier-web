...
const OrderList: React.FC = () => {
  ...
  const [link, setLink] = useState<PaginationState>({
    page: 1,
    total: 0,
    lastPage: 0,
    perPage: 10,
  ...
  const loadData = (
    ...
    setFetching(true);
    get("/details/api/", { page, status, ...form }).then((data) => {
      setFetching(false);
      setOrders(data.data);
      setLink({ ...data.links });
    });
      ...
      {isFetching ? (
        ...
        <div className="mb-10 order-list-grid">
          <div className="order-list-head">
            <div className="order-list-head-item">Detalles del pedido</div>
            <div className="order-list-head-item">Dirección de recojo</div>
            <div className="order-list-head-item">Dirección de envío</div>
            <div className="order-list-head-item">Estado</div>
            <div
              className="order-list-head-item"
          ...
          {link.total == 0 && (
            <div className="card text-muted">Sin pedidos registados</div>
          )}
        </div>
      )}
    </>
