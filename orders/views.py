...
def create_order_view(request):
    if request.user.is_client:
        ...
        if setting is None:
            messages.error(
                request, 'Falta una configuración, le recomedamos comunicarse con el administrador del sistema')
    ...
    if request.method == 'POST':
        if 'payed_image' in request.FILES and order.total > 0:
            if order.detail_set.count() == 0:
                messages.error(
                    request, 'El pedido no tiene ninguna dirección de envío')
            order.payed_order(
...
def assign_origins_to_driver_view(request):
    if request.method == 'POST':
        ...
        messages.success(
            request, 'Direccion(es) de recojo asignada(s) con éxito')
        ...
        for id in detail_ids:
            ...
            if not detail.is_assign_delivery:
                ...
                UnassignDeliveryAddress.objects.filter(
                    detail=detail).first().delete()
        messages.success(
            request, 'Direccion(es) de entrega asignada(s) con éxito')
...
def return_unassign_origin_view(request, pk):
    if request.method == 'GET':
        ...
        if not detail.is_unassign_origin:
            ...
            messages.success(
                request, 'La dirección de recojo, dejó de ser asignada al motorizado.')
            return redirect("orders:origins")
        messages.success(
            request, 'La dirección de recojo, ya dejó de ser asignada previamente.')
        return redirect("orders:origins")
    ...
    if request.method == 'GET':
        ...
        if not detail.is_unassing_delivery:
            ...
            AssignDeliveryAddress.objects.filter(
                detail=detail).first().delete()
            messages.success(
                request, 'La dirección de envío, dejó de ser asignada al motorizado.')
            return redirect("orders:deliveries")
        messages.success(
            request, 'La dirección de envío, ya dejó de ser asignada previamente.')
        return redirect("orders:deliveries")
...
class ExportOrdersExcelView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        headers = ['# Tracking', 'Cliente', 'Fecha', 'Dirección',
                   'Quien atenderá', 'Celular', 'Precio S/']
        name_sheet = 'Ordenes'
